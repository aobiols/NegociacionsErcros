...
@st.cache_data
def llegeix_i_filtra_dades(asset):

    print("LLEGINT DADES ................")

    ## LLegim les dades de tots els anys que tinguem ordenades
    all_files = glob.glob(os.path.join('.\\dades', "operacions_" + asset + "*.csv"))
    ...
    import_maxim = int(df['Import'].max())

    return df, import_maxim

def main(df_sencer, import_maxim):
    ...
    filtre_import = st.sidebar.slider('Importe', import_minim, import_maxim,(import_minim_parcial, import_maxim), format = '%d EUR' )
    ...
    data_final = df_sencer['datetime'].max().date()
    filtre_dates = st.sidebar.slider('Rango de fechas', data_inicial_tot, data_final, (data_inicial_parcial, data_final), format=format)
    data_maxima = np.datetime64(filtre_dates[1]) + np. timedelta64(1, 'D')
    mask_dates = (df_sencer['datetime'] >= np.datetime64(filtre_dates[0])) & (df_sencer['datetime'] <= data_maxima)
    df_sencer = df_sencer.loc[mask_dates]

    ### FILTRE PELA IMPORTS
    df_sencer = df_sencer[df_sencer['Import'].between(filtre_import[0], filtre_import[1])]

    # Calculem la Suma acumulada segons els filtres
    df_sencer['Cumsum_volum'] = df_sencer['Volum_signe'].cumsum()


    ### PREPAREM en un altre dataframe les dades que es pintaran a la gràfica
    df_a_pintar = df_sencer[['datetime','Cumsum_volum','Preu']]
    df_a_pintar = df_a_pintar.set_index('datetime')

    ################ TERCERA GRAFICA AMB HIGHCHARTS
    ...
    df_sencer = df_sencer[['Dia','Hora','Preu','Volum','Import','Operacio','Cumsum_volum']]
    df_sencer.columns = ['Dia','Hora','Precio','Volumen','Importe','Operación','Acumulado']

    df_sencer['Hora'] = df_sencer['Hora'].str[:8]

    ### ATENCIO STYLE NOMES ES POT APLICAR UN SOL COP , O COLOR O FORMAT !!!

    df_sencer = df_sencer.style.format({'Precio': '{:.2f}', 'Importe': '{:,.0f} EUR'})
    st.dataframe(df_sencer, hide_index=True, use_container_width=True)
    ...
    st.sidebar.info('\n\nEstudio de las negociaciones de Ercros para los compañeros de Nemer'
                    ...
                    '\n 2) La primera i la última negociacion del dia se consideran Subasta Inicial i Final (No cuentan)'
                    ...
                    '\n \n La idea es saber si hay una relacion entre las "grandes" compras/ventas y lo que sucederá en un futuro')
